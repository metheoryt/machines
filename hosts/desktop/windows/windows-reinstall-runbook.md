...
**Where this runbook + script live:** in the **`machines` repo** at `hosts/desktop/windows/` (this machine is `g16`). They're committed and pushed to `github.com/metheoryt/machines`, so they survive the wipe — after reinstall, `git clone` machines to get them back (no dependency on the broken OneDrive). Each backup run also drops standalone copies on the SSD: `R:\backup\windows-reinstall-runbook.md` and `R:\windows-reinstall\backup.ps1`. Run the script from the repo: `cd <your machines checkout>\hosts\desktop\windows`.
...
> **All of Phase 1 is automated by `backup.ps1`** (in this folder: `machines/hosts/desktop/windows/`). Run it from an elevated PowerShell — `cd <your machines checkout>\hosts\desktop\windows`, then `.\backup.ps1 -WhatIf` first, then `.\backup.ps1`. The steps below document what it does and why. Backup target is `R:\backup` (the Kingston USB SSD, Disk 1 — survives the wipe; the script hard-stops if pointed at Disk 0).
