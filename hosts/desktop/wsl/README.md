# desktop-wsl — the Ubuntu distro on `desktop`
...
```console
sudo install -Dm644 hosts/desktop/wsl/ssh-socket-override.conf \
...
distro's ports. Over the tailnet no rule is needed. Present on desktop as
...
desktop-wsl still does not restore it. `provision/wsl-fixes.sh` is where it
...
`wsl.exe -d desktop-wsl` through `desktop`, not over the network. While it was
