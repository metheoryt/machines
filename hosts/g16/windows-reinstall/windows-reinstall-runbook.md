...
**Where this runbook + script live:** in the **`machines` repo** at `hosts/g16/windows-reinstall/` (this machine is `g16`). They're committed and pushed to `github.com/metheoryt/machines`, so they survive the wipe — after reinstall, `git clone` machines to get them back (no dependency on the broken OneDrive). Each backup run also drops standalone copies on the SSD: `R:\backup\windows-reinstall-runbook.md` and `R:\windows-reinstall\backup.ps1`. Run the script from the repo: `cd <your machines checkout>\hosts\g16\windows-reinstall`.
...
> **All of Phase 1 is automated by `backup.ps1`** (in this folder: `machines/hosts/g16/windows-reinstall/`). Run it from an elevated PowerShell — `cd <your machines checkout>\hosts\g16\windows-reinstall`, then `.\backup.ps1 -WhatIf` first, then `.\backup.ps1`. The steps below document what it does and why. Backup target is `R:\backup` (the Kingston USB SSD, Disk 1 — survives the wipe; the script hard-stops if pointed at Disk 0).
...
> Keep the runbook readable while the PC is down — it's pushed to `github.com/metheoryt/machines` (`hosts/g16/windows-reinstall/`), so you can open it there from your phone or any device. The SSD also has a copy at `R:\backup\windows-reinstall-runbook.md`. (Do **not** rely on the broken OneDrive to deliver it.)
...
- [ ] **This runbook is readable from a device other than this PC** — confirm it's on `github.com/metheoryt/machines` (`hosts/g16/windows-reinstall/`, pushed) AND at `R:\backup\windows-reinstall-runbook.md` on the SSD (don't trust OneDrive to deliver it)
