...
> **⛔ Phase 1 IS NO LONGER AUTOMATED. `backup.ps1` was deleted 2026-07-31**
> (commit `1080828`), together with `restore.ps1`. This paragraph said "all of
> Phase 1 is automated" for six weeks after that stopped being true, which is the
> single most dangerous kind of stale line in this file: it is read immediately
> before a wipe. **Every step below is now something you do by hand**, and the
> steps are written out precisely because the script that used to do them is
> gone. Backup target is still `R:\backup` (the Kingston USB SSD, Disk 1 — it
> survives the wipe; Disk 0 is the one being erased, so check the disk number
> yourself, since nothing hard-stops you any more).
...
- **AIMP** — playlists (`PLS`), library, custom genres/moods, skins, `AIMP.ini` (the music *curation*; the files themselves are on server)
...
**Dropped on purpose (no backup):** Music (already on server), torrents, all Docker images + all Docker volumes (incl. qaz-law DB — re-ingest after reinstall), caches, `.venv`/`node_modules` in repos.
...
**Your manual job (the script won't do this):** copy `R:\backup\secrets\` to a **second location independent of the SSD** — server (`scp`/`rsync`), a password-manager attachment, or an encrypted archive emailed to yourself. Two independent copies of the few MB that can't be re-derived.
- [ ] Second copy of `secrets/` confirmed on server (or other independent location)
...
> 4. On the *other* machine(s): `git remote set-url origin git@github.com:metheoryt/machines.git` and rename the local clone dir to match. **On NixOS, the clone must live (or be symlinked) at `~/machines`** — `modules/home/claude.nix` reads `~/machines/agents` directly (updated 2026-07-07 from the transitional `~/nix` symlink indirection); a missing/dangling `~/machines` makes the next `home-manager` activation write broken symlinks (and the fish `~/machines` helpers break too). If your clone lives elsewhere, symlink it: `ln -sfn <clone-path> ~/machines`. `just switch` now hard-fails with a repoint hint if you skip this.
>
...
> Claude Code, `agents/bootstrap.sh`, and the machine-local `.claude` bits
> (`.credentials.json`, `settings.local.json`, `projects\`) restored out of a
> discovered backup.
>
> **It used to run `restore.ps1`, and that is gone (2026-07-31).** So the
> automatic half of Phase 4 no longer exists: repos, `.ssh` + its icacls perms,
> Downloads, the Obsidian vault and the cloud→`*-from-backup` reconcile are all
...
2. **SSH + configs (Windows):** copy `R:\backup\home\.ssh` → `C:\Users\<you>\.ssh`, then fix perms (icacls: remove inherited, grant your user only). Restore the other dotfiles (`.gitconfig`, `.wslconfig`, `.kube`, `.gcm`, `.config`, `.claude.json`, shell histories, etc.).
   - **Agent config (`.claude`) — bootstrap, don't copy verbatim:** run `provision\windows.ps1 -BackupRoot R:\backup` (add `-Work` if the work profile is used). One script enables **Developer Mode** (native symlinks fail without it — the agent config is all symlinks into this repo), sets **`core.symlinks=true`** (see below), ensures Git + Claude Code, runs `agents/bootstrap.sh` (via Git Bash — the `just agent-bootstrap` recipe and the bare `bash` on PATH both misbehave on Windows), and restores only the machine-local bits (`.credentials.json`, `settings.local.json`, `projects/`) without clobbering the freshly-bootstrapped symlink trees. On NixOS/macOS the equivalent is `just agent-bootstrap`.
     ...
     - **Verify by reading the file, never by re-running the checkout:** `wc -c CLAUDE.md` must match `AGENTS.md` (18778 bytes as of this writing), not 9.
   - **Background git-sync — do NOT register a `git-autofetch` task. There are exactly three Scheduled Tasks and `provision\windows.ps1` step 7 installs all of them** (`machines-converge`, `fleet-selfpull`, `dotfiles-sync`). The fetch-only refresher is POSIX-only by decision (2026-08-03): this clone is not a dev checkout — `git rev-list --count --all --not origin/main` is 0 in all three Windows-native repos, nothing on the box reads "behind by N" (no shell profile of any kind), and `fleet-selfpull` fast-forwards `main` outright rather than merely revealing that it is behind.
     **This step is why a phantom existed.** Its earlier text told you to register the task by hand from a deleted script's `.EXAMPLE` snippet, and someone did: the live task's fingerprint (S4U/Limited, a 3650-day `RepetitionDuration`, no `RandomDelay`) matches that snippet and matches nothing `windows.ps1` emits, which sets `Repetition.Duration = ''` and `RandomDelay = PT2M`. It then pointed at `scripts\git-autofetch.ps1`, a path that stopped existing when `50540a1` renamed it on 2026-07-07, so every run since returned `0xFFFD0000` — `powershell.exe`'s code for "the `-File` argument does not exist" — without ever starting git. Unregistered 2026-08-03.
     **And the old verify line is why nobody noticed for 27 days:** `Get-ScheduledTask git-autofetch` checks *presence*, and the task was present the whole time. Verify a scheduled job by its `Last Result` after an unattended fire, never by its existence:
     ```powershell
     # ≥12 min later, unattended — do NOT schtasks /run
     schtasks /query /tn fleet-selfpull /v /fo LIST | Select-String 'Last Result','Next Run'
     ```
     If Windows-native ever does become a box you commit on, port a fresh `provision\git-autofetch.ps1` from the **current** `tier_autofetch` heredoc (`provision/lib/tiers.sh`) — which has the `timeout` shim, the shallow-clone skip and the all-failed non-zero exit that the deleted version never got — and register it from `windows.ps1` step 7 at an offset **off** selfpull's 10-minute boundary, to avoid the `cannot lock ref` race that `provision/fleet-selfpull.sh:51-61` exists to absorb. Do not restore the old file from `nixos-final`.
3. **WSL:** install WSL + Ubuntu, then either
   ...
   - **`.wslconfig` — the VM caps and the freeze guard (host-side; nothing inside the distro can set these).** Since 2026-08-12 this file is **tracked in the dotfiles repo on the `g16` branch** — restore it from there, not by hand, and treat that copy as the source of truth (it carries the per-setting reasoning as comments, and the sync timer keeps it current). The block below is illustrative, like the `R:\backup` drive letters above: it is what the file should contain, so you can verify the restore, because getting it wrong wedges the VM:
     ...
     sparseVhd=true
     ```
     **Do not "tune" `memory=` down.** On 2026-08-09 the VM froze with `Wsl/Service/0x8007274c` because anonymous memory reached ~15.8 GB of the 16 GB cap *and* swap hit zero, after which an order-6 kernel allocation failed inside `vmbus_alloc_ring` and took out the vsock transport the WSL relay needs. A lower cap brings that wall **closer**. `autoMemoryReclaim` is `disabled` for the same reason — gradual reclaim hands guest memory back to the host, which is the wrong direction when the guest is what runs out. The cap was `16GB` at the time of that freeze and was raised to `40GB` on 2026-08-12 — the host has 63.6 GiB, so 40 still leaves ~23 GiB for Windows, and `memory=` is a ceiling rather than a reservation. Note it is VM-wide, shared by every distro plus `docker-desktop`, not per-distro. The guest-side half of the guard (kernel reserves + earlyoom) is installed by `provision/wsl-fixes.sh`; full diagnosis in qaz-code `docs/known-issues/sync-wsl-freeze.md`. Changes need `wsl --shutdown`.
...
**Already safe (not on SSD):** git repos pushed to GitHub · Music on server · Chrome/JetBrains via account sync.
