...
- **Orca is StablyAI's multi-agent IDE; its "workspaces" are orca-managed git
  ...
  `C:\Users\methe\orca\workspaces\...`). The CLI is Orca's own `orca` on the
  Windows host (the old WSL-side `~/.local/bin/orca` shim generated by
  `provision/orca-serve.sh` was removed 2026-07-21 — Orca now runs on Windows
  and opens the WSL project directly). Key commands: `orca repo add --path <p>`
  / `orca repo list` register a project repo; `orca worktree create --repo
  name:<r> --name <n> --base-branch <ref> [--setup skip]` materializes AND
  registers a worktree checkout; `orca worktree ps` / `orca worktree list
  --json` enumerate them. Prefer creating workspaces through the CLI so Orca's
  registry knows them — a hand-built `git worktree add` is invisible to Orca.
- **The repo/worktree registry is per-runtime, i.e. per host.** The WSL Orca
  runtime and the Windows one keep separate registries — a repo/workspace
  registered on Windows is unknown to WSL Orca until you `repo add` +
  `worktree create` it there. `orca repo list` reflects only the local runtime.
- **Gotchas when migrating/inspecting a worktree by hand:** (1) a worktree's
  `.git` is a *gitdir pointer* into the parent repo's `.git/worktrees/<n>`;
  across a Windows↔WSL boundary it holds a `C:/…` path that breaks on Linux —
  check status with the worktree's OWN gitdir (`GIT_DIR=<parent>/.git/worktrees/<n>`),
  NOT the parent repo's index, or you'll misread the branch-vs-main delta as
  fake "uncommitted modifications". (2) Per-worktree working state is gitignored
  (`.venv/`, `*.db`, `.env`, `.superpowers/`) — carry DBs/`.superpowers` over
  explicitly and REBUILD `.venv` per-platform (`uv sync`); never copy a Windows
  venv to Linux.
...
- **Personal project repos are LOCAL-ONLY git (no remote), e.g. `~/my/buton`.**
  Two machines' copies share history only because they were copied; they don't
  auto-sync. Move a branch between them with `git fetch <other-path> <branch>:<branch>`.
