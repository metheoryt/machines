...
- **Orca is StablyAI's multi-agent IDE; its "workspaces" are orca-managed git
  ...
  `C:\Users\methe\orca\workspaces\...`). The CLI is `~/.local/bin/orca` (an
  Electron/`ELECTRON_RUN_AS_NODE` shim generated by `provision/orca-serve.sh`;
  regenerate after an Orca upgrade). Key commands: `orca repo add --path <p>`
