...
| c | Windows profile user | Trust `fleet.json` `ssh.user`; `/mnt/c/Users/*/.claude/projects` scan as fallback | `ssh.user` (`methe`) is the authoritative Windows profile user; the scan covers drift / a wrong-user config. The WSL login user is `me` and is NOT in fleet.json — use bare `~` for the WSL root. |
