...
### Tasks 5–6 are now mostly one command

> **Superseded by `just provision-mac air` (commit `ad9e417`).** Steps 1–4 of Task 5 and Steps 1–2, 7 of Task 6 were written as manual `scutil` / Homebrew / `tailscale up` prose. Prose is how a Mac ends up half-provisioned, so they are now a script chain — design in `docs/superpowers/specs/2026-07-27-macos-one-command-provisioning-design.md`.
>
> ```bash
> git clone git@github.com:metheoryt/machines.git ~/machines
> cd ~/machines
> mkdir -p provision/secrets
> printf '%s' '<hskey-…>' > provision/secrets/authkey    # mint: ssh hub 'sudo headscale preauthkeys create --user 1 --expiration 2h'
> just provision-mac air --dry-run                       # preview, touches nothing
> just provision-mac air --authkey-file provision/secrets/authkey
> ```
>
> One sudo prompt, then: hostname → Homebrew → tailnet → 13 tiers → roles.
>
> **What the chain does NOT do**, and still needs a human — the manual steps below are kept for exactly these:
>
> | Still manual | Where |
> |---|---|
> | `gh auth login` to register the SSH keys | Task 6 Step 1b — **must happen before any further clone** |
> | Enrolling `id_fleet.pub` in `provision/fleet-authorized-keys` | Task 6 Step 1b |
> | Remote Login, if macOS blocks `systemsetup` | Task 5 Step 5 |
> | Work-repo clones + their gitignored `settings.local.json` | Task 6 Steps 3–4 |
> | Docker Desktop, VPN, `tsh` | Task 6 Step 5 |
> | Orca install + pairing `desktop` | Task 6 Step 8 |
> | The full-day soak gate | Task 6 Step 9 |
>
> The steps below remain accurate as a description of *what happens* and as the fallback when a single stage needs re-running — each is independently invocable (`bash provision/tailscale-mac.sh --hostname air …`).
...
| 2026-07-27 | `0353a3b` | `tier_fleet_ssh` — closes a gap that would have broken Task 5 Step 4. See below. |
| 2026-07-27 | `844f92d` | `fleet_hostname()` strips an mDNS/DNS suffix, so macOS reporting `air.local` no longer defeats `fleet_detect` / `fleet_profile_for_host`. |
| 2026-07-27 | `74c8bfc`, `ad9e417` | **Tasks 5–6 largely automated** — `just provision-mac air` chains hostname → Homebrew → tailnet → tiers → roles. See the box at the head of Task 5. |
