...
## 68f42627 · contradiction · /home/me/.claude/CLAUDE.md

- **action:** contradiction
- **scope:** shared → fleet-wide change
- **apply on:** g15 (observed here; re-check every box before changing the shared text)
- **target:** `/home/me/.claude/CLAUDE.md`
- **anchor:** `MANDATORY: Use Gortex MCP tools instead of Read/Grep/Glob` (L2, 5353 B)
- **why:** the section's own escape hatch fires on a box where the mandate is
  mechanically enforced, and tells the agent to do the one thing the hook blocks.
  Half A (L6): `@/home/me/.gortex/instructions/active.md` — **that path does not
  exist on g15**, so the import comes through empty. Half B (L11-14): *"If the
  Gortex import above came through empty, this box has no gortex… On such a box
  ignore the mandate above entirely and use the native Read/Grep/Glob tools."*
  But g15 **has** gortex: the binary is at `~/.local/bin/gortex`, the daemon was
  up at session start (v0.63.10, 5 repos indexed, cwd tracked), and the
  `PreToolUse` deny hook is live. An agent obeying half B reaches for Read and
  is blocked — the emptiness test is reading a missing file as "no gortex"
  instead of as "broken install".
- **evidence:** `ls ~/.gortex/instructions/` → *No such file or directory*, while
  `gortex instructions list` reports *dir: /home/me/.gortex/instructions* with
  profile `core` marked active (body 3061 B) · `~/.claude/CLAUDE.md`:2-14 ·
  session-start banner: *"Gortex daemon ready … Enforcement is active."*
- **bytes:** no change proposed by this run — **a human picks which half is true**
- **replacement:** (none — two candidate fixes, pick one:
  (a) repair the box — `gortex instructions switch core` / `gortex install` — and
      leave the text alone, in which case this is a host bug, not a memory bug; or
  (b) if an empty import is a legitimate fleet state, change the emptiness test to
      one that can tell "profile not provisioned" from "binary absent", e.g. gate
      on `command -v gortex` rather than on the import being empty.)
- **also observed this run:** the daemon went **unreachable** partway through and
  enforcement stood down to rule-only. That is a second way the box can be in
  half B's state while gortex is installed.
- **first seen:** 2026-09-11

## 2f658c5e · contradiction · /home/me/.claude/memory/global.md

- **action:** contradiction
- **scope:** shared → fleet-wide
- **apply on:** (decision only — the promotes are filed separately, per box)
- **target:** `/home/me/.claude/memory/global.md`
- **anchor:** `(fleet: shared stores vs origin/main)` — whole-branch finding, no heading
- **why:** `~/.claude/CLAUDE.md` asserts the shared stores are *"on `main`
  (shared, byte-identical everywhere)"*. **That is false on all six of them.**
  `origin/main` has not been touched since **2026-08-27** (15 days), so nothing
  has been promoted in that window, and the fleet has diverged into three
  clusters. This is not one box lagging — a branch *smaller* than main is just
  waiting for its sync tick; here **every live box is ahead of main**, on
  different content.
- **evidence:** `git --git-dir=$HOME/.dotfiles ls-tree -r --full-tree <ref> -- .claude/memory`,
  blob hashes (bytes):
  ...
  `git --git-dir=$HOME/.dotfiles log -1 --format=%ci main` → `2026-08-27 18:16:04 +0500`
- **what this costs:** a box that re-checks out from `main`, or air on its next
  sync, gets a `global.md` **24.5 KB smaller** than g15's and a `practices.md`
  **9.8 KB smaller** than desktop-wsl's. `values.md` on main (3284 B) is missing
  the 2.8 KB that three live boxes carry — including, per `core.md`, the
  *"never destroy the last copy of a secret"* non-negotiables.
- **bytes:** n/a — this item is the reconcile decision, not an edit
- **replacement:** (none — **a human picks**. Three clusters means a merge, not a
  promote: cluster A = main + the dead g15-wsl branch (the 2026-08-27 baseline),
  cluster B = latitude + hub + g15 (the fleet's working state), cluster C =
  desktop-wsl (furthest ahead on every personality facet and on core.md). Do NOT
  let a single `/dotfiles-promote` run pick a winner file-by-file without the
  hashes above on the table.)
- **first seen:** 2026-09-11

## 6d32518b · promote · /home/me/.claude/memory/personality/practices.md

- **action:** promote
- **scope:** shared (currently stranded on one branch)
- **apply on:** **desktop-wsl** — NOT this box. `/dream-apply` on g15 must refuse it.
- **target:** `/home/me/.claude/memory/personality/practices.md`
- **anchor:** `(branch: desktop-wsl)` — whole-file, per-branch finding
- **why:** desktop-wsl carries the largest block of shared memory that has reached
  no other machine. Five shared files there are ahead of `origin/main` and of every
  other box, **~17.8 KB in total**, and shared memory is only shared once promoted:
  | file | desktop-wsl | main | delta |
  |---|---|---|---|
  | practices.md | 31990 | 22141 | **+9849** |
  | values.md | 7034 | 3284 | +3750 |
  | tone.md | 10624 | 9116 | +1508 |
  | global.md | 122676 | 99341 | +23335 (but = latitude/hub, so cluster B, not unique) |
  | core.md | 3385 | 3004 | +381 — see the core.md budget item; 3385 B is **inside
  |  |  |  | the ~3500 B SessionStart truncation margin** |
  | .claude/CLAUDE.md | 9593 | 9187 | +406 |
  practices.md is a **behavioural** store — craft opinions that shape how every
  session writes code. Nine KB of it applying on one box and nowhere else is the
  failure the shared scope exists to prevent.
- **evidence:** `git --git-dir=$HOME/.dotfiles ls-tree -r --full-tree origin/desktop-wsl -- .claude/memory`
  → practices.md `621d4b12` vs main `c193248a`; branch tip 2026-09-11 (current, not stale)
- **bytes:** +9849 to main's practices.md (and +3750 values, +1508 tone) if promoted whole
- **replacement:** (none — the action is `/dotfiles-promote` **run on desktop-wsl**,
  reviewing each file. Read the diff first: some of the delta may be desktop-wsl
  specifics that belong in its `host-memory.md` (33529 B, the fleet's largest) rather
  than on `main`.)
- **first seen:** 2026-09-11

## 08ade540 · promote · /home/me/.claude/memory/personality/practices.md

- **action:** promote
- **scope:** shared (currently stranded on one branch)
- **apply on:** **air** — NOT this box.
- **target:** `/home/me/.claude/memory/personality/practices.md`
- **anchor:** `(branch: air)` — whole-file, per-branch finding
- **why:** air is the **primary dev box** and it is simultaneously ahead of main on
  two personality facets and behind it on everything else — the worst combination,
  because its next sync tick will overwrite nothing but will also never publish what
  it holds. Ahead: practices.md 23148 vs 22141 (**+1007**), tone.md 9644 vs 9116
  (**+528**). Behind: global.md 86373 vs main's 99341 and g15's 123859 — i.e. the
  primary dev box is reading a `global.md` **37 KB smaller** than this one.
- **evidence:** `ls-tree origin/air` → practices `3bedd872`, tone `9c0bd12b`,
  global `d231e355`; branch tip **2026-09-07**, four days stale, and `main` is
  *not* merged into it (`merge-base --is-ancestor main origin/air` → false,
  62 ahead / 1 behind)
- **bytes:** +1535 to main across the two facets
- **replacement:** (none — `/dotfiles-promote` **run on air**. Its unmerged `main`
  is the more urgent half: air has not taken a `main` merge, so a promote from
  there needs the merge resolved first.)
- **first seen:** 2026-09-11

## 6b30ae15 · promote · /home/me/.claude/memory/global.md

- **action:** promote
- **scope:** shared (currently stranded on this branch)
- **apply on:** **g15** — this box, so `/dream-apply` here may act on it.
- **target:** `/home/me/.claude/memory/global.md`
- **anchor:** `(branch: g15)` — whole-file, per-branch finding
- **why:** g15 holds the largest `global.md` in the fleet — 123859 B against
  122676 on latitude/hub/desktop-wsl and 99341 on main. The **1183 B** over cluster B
  is memory written here that has reached no other box; the 24518 B over main is the
  fleet-wide backlog covered by the divergence item.
- **evidence:** `ls-tree origin/g15` → global.md `7626200b` (123859) vs
  `d5f0a8fd` (122676) on latitude, hub and desktop-wsl, vs `dd800fdc` (99341) on main.
  Branch tip 2026-09-11 (current). `values.md`/`habits.md` here are cluster B, not unique.
- **bytes:** +1183 over the fleet's working copy
- **replacement:** (none — `/dotfiles-promote` on g15. Diff against `d5f0a8fd`
  first and promote only the 1183 B that is genuinely global: this box's recent
  work was the Ubuntu reinstall, and a reinstall detail belongs in its
  `host-memory.md`, not in a store every machine loads.)
- **first seen:** 2026-09-11
...
## 1ab6ee45 · contradiction · /home/me/.claude/memory/global.md

- **action:** contradiction · **scope:** shared → fleet-wide
- **why:** two bullets send work at machines that no longer exist in that shape. Accept/reject each part independently.

### Part 1 — `/ship` and kb-refresh reach g15 as a "Windows-native clone"
`fleet.json` has exactly one `platform: windows` entry (`desktop`, line 17); g15 is
`platform: debian` after the 2026-09-07 Ubuntu reinstall. This dispatches down a
path that no longer exists on that box.
- **evidence:** global.md:241-259 · `fleet.json`:17,24
- **bytes:** 1372 → 1363
- **replacement:**
- **`/ship` (`fleet-pull.sh`) and kb-refresh (`fleet-gather.sh`) reach EVERY
  ...
  `fd_wsl_hosts`). **Windows-native clones** are reached by launching Git Bash
  (`C:\Program Files\Git\bin\bash.exe`) through PowerShell's call operator
  (`&`), keyed on `platform: windows` in `fleet.json` — this runs the remote
  script against the Windows clone (`$HOME=/c/Users/<winuser>`), not the WSL
  default distro (live-verified 2026-07-22: `fd_run desktop windows` →
  `HOME=/c/Users/methe` + the Windows clone's commit). **`desktop` is the only
  `platform: windows` member left** — `g15` was reinstalled as Ubuntu 2026-09-07
  and is `platform: debian`, so it is reached over plain ssh like any posix box.
  **Self-declared WSL hosts** are discovered by their Windows parent via
  `wsl -l -q` + reading each distro's gitignored `fleet.local.json`; any distro
  with `.self.fleet == true` is pulled directly at `<nickname>.gg.ez`. WSL
  distros are NEVER added to `fleet.json` — they self-declare via
  `just provision-wsl <nickname>`. `machines` is located canonical-path-first
...
### Part 2 — two stale OS labels in the login-shell bullet
`latitude` is called a "NixOS/Linux member" (Debian 13 since 2026-08-01) and
`g15`=g513ie a Windows/PowerShell member (Ubuntu since 2026-09-07). The operative
facts — fish on the Linux boxes, PowerShell on Windows, always force bash — survive.
- **evidence:** global.md:213-228 · `fleet.json`:24
- **bytes:** 1228 → 833
- **replacement:**
- **The remote login shell differs by box.** Debian/Ubuntu members (`latitude`,
  `g15`, `hub`) log in to **fish**, which chokes on `$(...)` / POSIX-test syntax
  passed as `ssh host '<script>'` (fails silently / non-zero). The **Windows
  fleet member `desktop` (=g614jv) SSHes into PowerShell**, which chokes on
  `&&` and shell quoting. Either way, force bash: `ssh host bash -s < script.sh`
  (piping a script file is the most robust). On the Windows box `bash -s`/`bash -lc`
  dispatches to WSL bash — specifically the **DEFAULT WSL distro** (`bash.exe`),
  which on `desktop` is **`desktop-wsl`** (`$HOME=/home/me`). A non-default
  distro is never reached this way. `g15` was a Windows/PowerShell member until
  2026-09-07 and is not one now; `latitude` was NixOS until 2026-08-01 and the
  fish half is what survived that.
- **first seen:** 2026-09-11

## 4e3d386c · contradiction · /home/me/.claude/memory/global.md

- **action:** contradiction · **scope:** shared → fleet-wide
- **why:** both named arms are **inverted** against live source. `provision/linux.sh:118`
  omits `ssh_accounts` for the `hub` profile deliberately, while `:182` (the `server`
  profile = latitude) includes it — yet global.md lists `hub` as a wired tier box and
  `latitude` as "NixOS, no GitHub block at all", citing `modules/home/ssh.nix`, deleted
  2026-08-01. A reader trusting this debugs the wrong box.
- **evidence:** global.md:95-103 · `provision/linux.sh`:115-118,182 · AGENTS.md *The NixOS tree is gone*
- **bytes:** 715 → 730
- **replacement:**
- **The convention is NOT fleet-wide — there are three implementations.**
  (1) POSIX tier boxes (`air` on `workstation`, `latitude` on `server`, the WSL
  distros): `tier_ssh_accounts` wires the aliases + identities described below.
  (2) **Windows** (`desktop`): hand-typed `githubcyphy`, written by nothing and
  reached by no rename. (3) **`hub` has no GitHub block at all** — `linux.sh`'s
  `hub` profile omits `ssh_accounts` deliberately, because it would overwrite
  hub's `~/.ssh/config` with `IdentitiesOnly` on a fresh unregistered key and
  kill its GitHub auth. Fine while hub has no cyphy671 repos; it would silently
  offer the wrong key if it ever did. Don't assume an alias exists on a box
  without checking.
- **first seen:** 2026-09-11

## f76e1f4f · demote · /home/me/.claude/memory/global.md

- **action:** demote · **scope:** shared → repo:machines
- **survives:** `/home/me/machines/.claude/memory/project.md`, beside the existing kb-refresh block (L1362-1373)
- **why:** 10.2 KB about `agents/plugin/skills/kb-refresh/fleet-gather.sh` — a script
  that lives in `machines`, whose architecture project.md already documents — plus
  airdrome/qaz-code cron-state observations that are *in* other repos but *about* the
  machines-owned skill. This is 8% of a store every session on every box loads.
- **evidence:** global.md:348-492 · `machines/.claude/memory/project.md`:1362-1373, 274-277
- **carry first:** the WSL self-exclusion consequence is **NOT** in project.md
  (project.md:274-277 has only the generic hostname-probe rule) — it must be carried
  across or it is lost. It is kept in the replacement below, not in the move.
- **bytes:** 10193 → 816 in global (−9377); +~9.4 KB in machines/project.md
- **replacement:** (the body moves VERBATIM to `machines/.claude/memory/project.md`; what stays in global.md is:)
...
## f6378ffc · demote · /home/me/.claude/memory/global.md

- **action:** demote · **scope:** shared → repo:machines
- **survives:** `/home/me/machines/.claude/memory/project.md`, beside its `SKIP unreachable` block (L709-724)
- **why:** two blocks that are `machines`-repo mechanism, not fleet-wide truth. Accept/reject each part independently.

### Part 1 — the `air`-missing-from-the-block case (1255 → 439)
A `machines` mechanism story (`ssh_wsl_render_config`, `provision/ssh-wsl.sh`,
`fleet.json` membership). The two-failure-mode triage tell is the portable half and stays.
- **evidence:** global.md:155-170 · `machines/.claude/memory/project.md`:709-724
- **replacement:** (body moves VERBATIM to machines/project.md; what stays in global.md is:)
- **A generated `~/.ssh/config` fleet block goes stale silently, and a missing
  member reads as "unreachable" — re-render it after any `fleet.json`
  membership change.** Mechanism and the 2026-07-30 `air` case:
  `machines/.claude/memory/project.md`.
  - **Distinguishing tell for the two SSH failure modes:** a stale host key's
    error names an offending `known_hosts` line; a missing `Host` stanza / no
    identity names nothing.

### Part 2 — `provision/ssh-wsl.sh` internals (716 → 0)
Entirely about one script in the `machines` repo (which half needs sudo, how to
splice the jq-rendered config between the markers) plus a one-box 2026-07-30
observation on desktop-ubuntu26. Nothing here is true of every machine.
- **evidence:** global.md:183-191 · `provision/ssh-wsl.sh`
- **replacement:** (none in global — the block moves VERBATIM into machines/project.md next to the fleet-ssh renderer block)
- **first seen:** 2026-09-11
...
## c83fa8c8 · delete · /home/me/.claude/CLAUDE.md

- **action:** delete · **scope:** shared → fleet-wide (loaded in EVERY session on EVERY box)
- **why:** this bullet names three things that exist nowhere in the fleet —
  `modules/home/claude.nix`'s activation script, `nixos-rebuild`, and "this flake",
  all deleted 2026-08-01 in `f3d63b2` with the last Nix host. **The guard itself**
  ("do NOT run X to apply a memory edit") is a live incident and is **KEPT**, with its
  command names swapped to the surviving `just agent-bootstrap`. Also trims
  "Propagation changed with the handover" — narrative addressed to a reader who
  remembers the old system, not a rule.
- **evidence:** `~/.claude/CLAUDE.md`:125-139 · in `~/machines`: `test -d modules` → missing,
  `test -f flake.nix` → missing, `find . -name '*.nix'` → nothing, `just --list | grep switch` → no match
- **bytes:** 1104 → 846
- **replacement:**
  `bootstrap.sh` touches them only to fan the OTHER profiles (`~/.codex`, each
  `~/.claude-<postfix>`) out at this primary profile — `just agent-bootstrap` /
  `just agent-bootstrap-profile <postfix>`.
  **Editing a memory takes effect immediately — a plain write to a dotfiles-tracked
  file. Do NOT run `just agent-bootstrap` to "apply" it.**
  The 10-minute `dotfiles-sync` timer commits the write to this machine's branch
  on its own (debounced — one editing burst, one commit) and pushes it. Getting
  it onto `main` so every box sees it is a manual `/dotfiles-promote`;
  `/dotfiles-sync` forces the commit+push now.
- **Per-project** memory lives *inside the target repo* (its `CLAUDE.md` /
  ...
  from the working directory — each repo carries its own.
- **first seen:** 2026-09-11

## e699b706 · delete · /home/me/.claude/CLAUDE.md

- **action:** delete · **scope:** shared → fleet-wide (loaded in EVERY session on EVERY box)
- **why:** a second dead-command site, separate from the Wiring bullet: `just switch` no
  longer exists in the `machines` justfile, so this sentence tells every session on every
  box to wait on a recipe that cannot run. `just agent-bootstrap-profile <postfix>` is
  the live half and is kept. Also drops the `d48c09a` parenthetical — a changelog note
  about a work profile that was folded back in and no longer exists.
- **evidence:** `~/.claude/CLAUDE.md`:57-64 · `just --list` in `~/machines` shows only
  `agent-bootstrap` and `agent-bootstrap-profile postfix`; no `switch` recipe ·
  `machines/AGENTS.md` lists `nixos-rebuild`, `nix flake check` and `just quick` as commands that do not exist
- **anchor note:** `dream.sh index` emits no `##` for this region — it falls under the H1
  `# Persistent memory (synced & version-controlled)` at L39, and the nearest indexed
  `##` is the gortex heading at L2. See the `skill` item on anchor granularity.
- **bytes:** 607 → 531
- **replacement:**
...
`settings.<postfix>.json` → `~/.claude-<postfix>`. Drop a new
`settings.<postfix>.json` into `agents/` and the next
`just agent-bootstrap-profile <postfix>` provisions `~/.claude-<postfix>` with
the full shared set — no wiring edit needed. Each profile's machine-local
`settings.local.json` (where `gortex install` writes its hooks) is owned by
neither mechanism and never committed —
- **first seen:** 2026-09-11
...
## 7f7db67c · compress · /home/me/.claude/memory/core.md

- **action:** compress · **scope:** shared → fleet-wide · **URGENT per the standing check** (file is 3004 B, over the 3000 threshold)
- **TWO MEASURED CORRECTIONS TO THE PREMISE — read these before acting:**
  1. **The loader STRIPS HTML comments**, so the 597-B header block never reached the
     model. The file emits **2442 B**, under the ~3.4 KB hook cap, so `mem_warn_if_over`
     was never firing and **the 2 KB rule is a file-byte convention** from
     `~/.claude/CLAUDE.md:86`, not live truncation. Real injected saving is
     **2442 → 1937**, not 3004 → 1997.
  2. **`register-reinject.sh` re-injects only the FIRST LINE of each marked bullet**, per
     turn. So every marked bullet in the replacement is rewritten to carry its whole rule
     on line 1 — verified by running the hook against the replacement text below.
- **why:** Register is the largest indexed section (1472 B) and gives back the most of any
  anchorable section (~320 B; `User` ~137, `Non-negotiable` ~68).
- **evidence:** core.md:1-51 · `bash global-memory-load.sh ~/.claude core | wc -c` → 2442
  (file 3004) · `bash register-reinject.sh` emits 4 truncated first lines
- **nothing is lost — each cut is already recorded elsewhere:** «с бэкграундом в ИБ» /
  «N лет» wording at global.md:19-25 · «считаю, что…» first-person voice at global.md:30 ·
  "Claude talking to himself" at tone.md:54 · noslopgrenade.com at practices.md:248
- **one honest cost:** the only voice pointer left is «голос → global.md `## User`», so
  the tone.md pointer no longer implies the «считаю, что…» rule.
- **bytes:** 3004 → 1997 (injected 2442 → 1937)
- **replacement:**
# Memory core

<!-- Verbatim per session; REGISTER-REINJECT re-injects line 1 per bullet. -->
...
## 01164472 · promote · /home/me/.claude/memory/core.md

- **action:** promote · **scope:** shared → fleet-wide (values.md → core.md)
- **why:** "Не спрашивать разрешения на безопасное действие" is a direct, **twice-repeated**
  instruction from him, and values.md records that it was already written down and still
  not applied — *«Правило прямо над этим бюллетнем уже говорило то же самое; я его не
  применил»*. That is the exact diagnosis for a rule living only in a **non-injected**
  facet. It belongs with the other non-negotiables, which do inject.
- **evidence:** values.md:33-48 (*«сноси, и можешь даже не спрашивать для настолько
  безопасных действий»*; *«и про него тоже не спрашивай, когда очевидно»*) ·
  `grep -n "не спрашив\|permission" core.md` → no match
- **honest cost:** applied **on top of** the core.md compress item, this puts core.md at
  2163 B / 2103 B injected — over the 2 KB convention, still well under the ~3.4 KB hook
  cap. If the convention is held strictly, the compensating cut is «не специализация»
  from `## User` (covered at global.md:19-25), worth ~90 B.
- **bytes:** 484 → 650
- **replacement:**
## Non-negotiable (personality/values.md)
...
## 1d51905a · contradiction · /home/me/.claude/memory/personality/tone.md

- **action:** contradiction · **scope:** shared → fleet-wide
- **why:** the final bullet asserts **as verified fact** that no re-inject hook exists
  anywhere on disk. **It does now:** `agents/plugin/hooks/register-reinject.sh` is wired
  as a `UserPromptSubmit` hook in the cyphy plugin's `hooks.json` and emits the four
  marked core.md bullets every turn. The bullet's *conclusion* (this file is archive;
  what must bind goes in core.md `## Register`) is still right, but its stated
  **mechanism** is wrong — which would mislead the next editor into thinking the marked
  block is inert.
- **evidence:** tone.md:99-110 · `machines/agents/plugin/hooks/hooks.json`:29-38
  (`UserPromptSubmit` → `register-reinject.sh`) · live run prints the four marked bullets
- **bytes:** 945 → 652
- **replacement:**
- **NOT re-injected. This section is archive, not live** (verified 2026-08-24,
  re-checked 2026-09-11). The SessionStart loader injects `core.md` verbatim plus
  an *index* of this file; nothing here reaches a session unless opened
  deliberately. **Consequence: every calibration recorded here after 2026-08-04
  has not been binding** — which is why the same brevity rule kept coming back.
- **What binds is `core.md`'s `## Register`, and specifically the FIRST LINE of
  each bullet between the `REGISTER-REINJECT` markers.** That hook exists now —
  `agents/plugin/hooks/register-reinject.sh`, wired as `UserPromptSubmit` in the
  cyphy plugin's `hooks.json` — and it truncates each bullet to its first line.
  So a rule whose first line does not stand alone is a rule that re-injects as a
  fragment. Keep this file as the detail behind those bullets, and keep the two
  consistent.
- **first seen:** 2026-09-11

## 3f6beb1c · contradiction · /home/me/.claude/memory/personality/habits.md

- **action:** contradiction · **scope:** shared → fleet-wide
- **why:** the rule names `services.gitAutoFetch` **on NixOS** as the mechanism that keeps
  "behind by N" fresh. The fleet has had no NixOS host since 2026-08-01 and the flake was
  deleted; on Debian/macOS boxes that service does not exist, so the rule tells a reader
  to **trust a fetch that may not be running**. The behaviour (check before acting,
  pull --rebase if behind) is unaffected.
- **evidence:** habits.md:14-19 · `machines/AGENTS.md` *The NixOS tree is gone* (`f3d63b2`)
- **bytes:** 480 → 452
- **replacement:**
  - **Before acting on code:** a background fetch timer runs every ~10 min where
    one is installed (Windows: the `git-autofetch` Scheduled Task), so `git
    status` / the prompt may already show "behind by N". Check that — and on a
    box with no such timer, fetch first. If behind, pull+rebase before working
    (`git pull --rebase`). Start from an up-to-date base — never commit on a
    stale branch. The timer is fetch-only (refs); the pull is always yours.
- **first seen:** 2026-09-11

## 51c0b8a1 · compress · /home/me/.claude/memory/personality/habits.md

- **action:** compress · **scope:** shared → fleet-wide
- **why:** **a structural defect, not verbosity.** The ritual's own instructions ("Skip
  anything derivable from code/git history…", "Keep the step quiet") are buried mid-
  paragraph inside the *third* bullet, which is an honest caveat about why the gortex
  `remember` rule has not worked. A reader who takes the caveat as commentary and skips it
  loses two directives. This moves the rule text back to the rule and leaves the caveat
  intact as a caveat — **no rule is shortened**.
- **evidence:** habits.md:49-56 (the directives begin mid-line at 54, after "the fix is mechanical … not another line of prose.")
- **bytes:** 630 → 651
- **replacement:**
- **Skip anything derivable from code/git history, or that only matters to the
  current conversation. Keep the step quiet** — do the reflection and the writes,
  then report in a couple of notes, not a page-long summary.
- **Honest caveat, so this is not mistaken for a mechanism.** The gortex
  instruction profile already carries the `remember` instruction verbatim, in
  every session, and the store was still empty — so repetition is not what was
  missing. Treat a bare `remember` rule as intent; if it is still empty at the
  next audit, the fix is mechanical (a hook, or a step inside a skill that
  backfills), not another line of prose.
- **first seen:** 2026-09-11

## 4331ce50 · dedupe · /home/me/.claude/memory/personality/practices.md

- **action:** dedupe · **scope:** shared → fleet-wide
- **survives:** `~/.claude/memory/personality/tone.md` `## Design & spec documents`
- **why:** the "Write design/research docs for the engineer reading them" bullet restates
  tone.md's whole section — same audience, same cuts (restated context, hedging, formal
  scaffolding), same keep-list. It is a **voice** rule sitting in the **craft** facet, and
  specifically in the gortex-tuned subsection, where it has nothing to do with the code
  graph. tone.md's copy survives because it carries the 2026-07-10 CFT-4966 measurement
  (29 KB → 18 KB).
- **evidence:** practices.md:58-63 · tone.md:112-125
- **bytes:** 394 → 107
- **replacement:**
- **Design/research docs** — voice rules live in `tone.md` `## Design & spec
  documents`; read it before drafting one.
- **first seen:** 2026-09-11
