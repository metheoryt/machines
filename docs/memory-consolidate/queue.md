...
> **Branch and machine names changed 2026-09-12.** Any row below whose
> destination reads `desktop-wsl` means the dotfiles branch now called
> **`g16-wsl`**, and `desktop` means **`g16`** — the machine was renamed
> (machines `c04dc49`) and the old refs are deleted on origin. The `apply on:`
> lines have been updated in place; quoted evidence, literal paths
> (`backup/desktop-wsl/`, `hosts/desktop/`) and the WSL distro's Windows
> registration name — still `desktop-wsl`, deliberately — are left as written.
