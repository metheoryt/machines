...
- ~~**Why `check-before: true` never executes.**~~ **ANSWERED 2026-08-05: the placement hypothesis was correct, and `show` was the wrong instrument to test it with.** The key belongs under `backup:`; at profile level resticprofile 0.33.1 parses it, stores it, and echoes it back from `show` while never running it — so `show` resolving `check-before: true` was never evidence of anything. Counting issued `restic` commands is the test that discriminates: 4 scheduled `latitude` runs issued 4 `init` / 4 `backup` / 4 `forget` / **0 `check`**, and moving the key under `backup:` produced `init → check → backup` live. Fixed for `latitude` (`vps` `089ac3b`); `wsl` deliberately left alone until desktop-wsl can be reached to measure the cost of a daily check over the tailnet. **Generalise the method, not the key: a guard verified only by `show` is unverified.**
