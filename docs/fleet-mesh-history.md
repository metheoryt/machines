# Fleet mesh + migration history (archived)

<!-- Archived from .claude/memory/project.md on 2026-07-24 (against dd3d74b) by kb-refresh.
     Dead history — NOT session-loaded. The AWG mesh was retired 2026-07-17 (fleet on
     Headscale/Tailscale); the unified-provisioner + SSH-over-tailnet + fleet-rename efforts
     below are COMPLETE one-time migrations. Kept for rationale, not as live memory. -->

## AWG mesh per-box rollout + g614jv enrollment (retired)
...
## SSH-over-tailnet + fleet-hosts machinery (code-complete, then superseded)
...
## Fleet rename to consistent names + MagicDNS adoption (DONE, Layers 1-3)
...
## AWG hub peer map + mesh 'codified/partially activated' (retired)
...
## Unified fleet provisioner Phases 0-5b + mesh activation checklist (mesh phases retired)
