...
## Каналы внутри Казахстана — вопрос закрыт, не переоткрывать (2026-09-12)

- **Домашний интернет в РК: безлимитный, быстрый в обе стороны (до 50 Мбит/с),
  стабильный.** Владелец: «к интернету претензий нет и не будем их искать». Это
  относится и к Алматы, и к деревне под Карагандой. **Не строить оценок,
  упирающихся в аплинк, и не заводить замеры скорости как задачу.**
- **Заливка в immich проксируется через `hub`** (телефон → hub → latitude). Трафик
  на ps.kz безлимитный — их собственные материалы заявляют unlimited traffic на
  линейке VPS. Линк hub 20–40 Мбит/с. **Денежного довода против DERP-реле больше
  нет**, так что формулировка в
  `docs/superpowers/specs/2026-09-12-village-offsite-backup-design.md`
  («реле идёт через DERP на hub — платный VPS в Алматы») неверна наполовину.
- Порядок величин, чтобы не пересчитывать: 50 ТБ уезжают из дома за ~3,5 месяца
  на 50 Мбит/с, ~5 месяцев на 30 Мбит/с. Это приемлемо, вопрос закрыт.
- **Горизонт роста — десятки ТБ, «хоть 50».** Медиафайлы тяжелеют, съёмки растут,
  планируется подключение родственников к immich.cyphy.kz. Строчка спеки «steady
...
## Память флота: что стоит в каждой машине (измерено 2026-09-12)

Снято по ssh: `sudo -n dmidecode -t 17` (latitude), `system_profiler
SPMemoryDataType` (air), `Get-CimInstance Win32_PhysicalMemory` (g16),
со слов владельца (g15 — dmidecode там требует пароль, `sudo -n` отказывает).

| Машина | Что стоит | Итого | Слоты |
|---|---|---|---|
| **latitude** | Hynix `HMAA2GS6CJR8N-XN` 16 ГБ + Samsung `M471A1G44AB0-CWE` 8 ГБ, **DDR4-3200 SO-DIMM** | 24 ГБ | оба заняты |
| **g15** | 2 × Kingston 16 ГБ **DDR4-3200 SO-DIMM** | 32 ГБ | оба заняты |
| **g16** | 2 × G.Skill `F5-5600S4040A32G` 32 ГБ **DDR5-5600** (работает на 4800) | 64 ГБ | оба заняты |
| **air** | Samsung 16 ГБ **LPDDR5, распаяна** | 16 ГБ | слотов нет |

**Следствие для выбора железа в деревню.** Aoostar WTR Pro берёт ту же
**DDR4-3200 SO-DIMM**, что latitude и g15. То есть одна запасная планка
закрывает три машины сразу, и при отказе памяти замена лежит дома, а не едет
из Китая. Любой кандидат на DDR5 (ODROID-H5, CWWK, Minisforum, Aoostar WTR Max)
этого не даёт — там формат другой и запас пришлось бы держать отдельный.
Это самостоятельный аргумент за DDR4-платформу, помимо разницы в цене планок
(б/у 16 ГБ DDR4 SO-DIMM на OLX — 25 000 ₸, новая DDR5 8 ГБ в DNS — 86 440 ₸).

**Донорство памяти с g15 рассмотрено и отклонено (2026-09-12).** Там ровно
2×16, так что вынуть одну — это оставить рабочей машине 16 ГБ И уронить её
в одноканальный режим. g15 к тому же `memory_publisher` флота (Phase B
memory-harvest — агентские сессии). Экономия 25 000 ₸ того не стоит.

**Запас памяти на latitude (измерено 2026-09-12, uptime 1 день):** из 24 ГБ
занято 5,6, доступно 17, `Committed_AS` 12,3 ГБ, **swap не тронут вообще**
(1 МиБ из 14 ГБ). Самый прожорливый процесс — qbittorrent-nox, 3 ГБ; immich,
jellyfin и каждый *arr — по 230–540 МБ. Практический вывод: **latitude сегодня
памятью не ограничен**, и на 16 ГБ он бы тоже поместился (12,3 < 16), просто
кэш упал бы вдвое. Так что план «вынуть из latitude 8 ГБ в деревню, а latitude
потом доукомплектовать до 2×16» технически рабочий — но +8 ГБ на latitude
сегодня не покупают ничего измеримого.
