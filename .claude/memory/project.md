...
- **`/mnt/xs` в `/etc/fstab` закомментирован** (диск физически снят): живой
  ...
  -go`. Латентный дефект самого скрипта — он гоняет `findmnt --verify` по
  всему файлу, так что любая посторонняя протухшая строка вырубает главный
  guard фленки. Правильная проверка — «нет НОВЫХ ошибок против текущего
  файла»; отложено осознанно.
- Старая копия на `/mnt/servarr` (sdb2, HGST 931 G) **не тронута**. Освобождать
  только после проверки, что в qBittorrent нет errored-торрентов; дальше на неё
  ложится вторая копия закрытого архива 1970–2024 вместо снятого `/mnt/xs`.
...
- **`du -sb` — не критерий совпадения двух ext4-деревьев.** Он суммирует и
  `st_size` каталогов, а выросший каталог с удалёнными записями размечен иначе,
  чем его свежая копия: 2156 каталогов источника = 9 011 200 байт, свежие копии
  в среднем на ~20 байт меньше каждая. Гейт `archive-mirror.sh` требовал точного
  равенства `du -sb` (наследство от exfat-приёмника) и напечатал бы
  `ARCHIVE MIRROR INCOMPLETE` на полной копии через 2.5 часа — а перезапуск
  сказал бы то же самое, не найдя что исправлять. Сравнивать число обычных
  файлов и сумму их размеров; `du` держать как справку, не как решение.
