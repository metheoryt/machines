...
- **Прогон всех сюит циклом `while read … done < <(find …)` без `</dev/null`
  проглотил 19 из 52**: тест, читающий stdin, выедает остаток подстановки
  процесса. `just` на desktop-wsl нет, так что цикл руками — но с редиректом.
